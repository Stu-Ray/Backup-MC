...
log_file    =   "../Dataset/log.csv"

# 获取Transformer训练和测试所需的数据集
def getTransformerDatasets(logFile, modelPath="./Model/", k_value = 2):
    global v_size
    # 初始数据集
...
if __name__ == '__main__':
    ...
    for k_value in range(2,3):
        transactions, transaction_cache = getTransformerDatasets(log_file, model_path, k_value)
        if print_log:
            print("------------------- K = " + str(k_value) + " -------------------")
        X_data = []
        y_data = []
        ...
        output_filename    =   "../Output/Text/Transformer_Data_cache.csv"
